...
from ..utils.numpy_helpers import ensure_numpy_type, is_numpy
...
def convert_reduce_min(node, params, layers, lambda_func, node_name, keras_name):
    ...
    """
    if len(node.input) != 1:
        assert AttributeError("More than 1 input for reduce min layer.")

    input_0 = ensure_tf_type(layers[node.input[0]], name="%s_const" % keras_name)

    def target_layer(x, axis=params["axes"], keepdims=params["keepdims"]):
        return K.min(x, keepdims=(keepdims == 1), axis=axis)
