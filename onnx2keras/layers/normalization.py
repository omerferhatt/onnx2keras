...
@register_converter("BatchNormalization")
def convert_batchnorm(node, params, layers, lambda_func, node_name, keras_name):
    ...
    if len(weights) == 2:
        ...
        bn = keras.layers.BatchNormalization(
            axis=1, momentum=momentum, epsilon=eps, weights=weights, name=keras_name
        )

    layers[node_name] = bn(input_0)


@register_converter("InstanceNormalization")
...
def convert_lrn(node, params, layers, lambda_func, node_name, keras_name):
    ...
    logger.debug(
        "LRN can't be tested with PyTorch exporter, so the support is experimental."
    )
